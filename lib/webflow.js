...
// Collection schema (slugs): name, slug, submission-id, active (Switch),
// status (Option: pending | paid | failed), stripe-session-id, stripe-payment-intent,
// paid-at (DateTime), amount-cents (Number), company-name, contact-name,
// contact-email (Email), contact-phone, submitted-at (DateTime), website (Link),
// full-data (RichText)
...
// `active` controls the Active switch (true once payment is confirmed, false for
// free/unpaid submissions) and `status` must be one of the collection's Option
